// components/faqs/faqs-client-view.tsx
'use client';
...
import type { FAQ, FAQCategory } from '@/lib/utils/faq-adapter';
...
export function FaqsClientView({
  ...
  // Generate FAQ structured data from all FAQs (for SEO)
  const faqSchema = generateFAQSchema(
    initialFaqs.map((faq) => ({ question: faq.question, answer: faq.answer }))
  );

  // Build category options including "All"
  const categoryOptions: (FAQCategory | 'All')[] = [
    'All',
    ...categories.map((c) => c.name as FAQCategory),
  ];
