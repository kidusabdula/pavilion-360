...
import { updateBlogCategorySchema } from '@/lib/schemas/blog-category.schema';
...
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    if (error) {
      if (error.code === 'PGRST116') throw ApiError.notFound('Blog category');
...
export async function PUT(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const validation = updateBlogCategorySchema.safeParse({ id, ...body });
