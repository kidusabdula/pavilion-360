...
import { useRentals, useDeleteRental, useRentalCategories } from '@/hooks/cms/use-rentals';
...
export default function RentalsPage() {
  const [deleteTarget, setDeleteTarget] = useState<RentalWithCategory | null>(null);
  const [categoryFilter, setCategoryFilter] = useState<string>('');
  
  const { data, isLoading, error } = useRentals({ category: categoryFilter || undefined });
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast.error(err instanceof Error ? err.message : 'Failed to delete rental');
    }
  };
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="rounded-lg border border-red-500/20 bg-red-500/10 p-6 text-center">
          <p className="text-red-500">Failed to load rentals: {error.message}</p>
        </div>
      </div>
    );
  }
